...
class ControllerPaymentUnitpay extends Controller {
	...
	public function index() {
		...
		// URL
		$this->data['copy_result_url'] 	= HTTP_CATALOG . 'index.php?route=payment/unitpay/callback';
		$this->data['copy_success_url']	= HTTP_CATALOG . 'index.php?route=payment/unitpay/success';
		$this->data['copy_fail_url'] 	= HTTP_CATALOG . 'index.php?route=payment/unitpay/fail';
...
$this->data['action'] = $this->url->link('payment/unitpay', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['unitpay_login'])) {
			$this->data['unitpay_login'] = $this->request->post['unitpay_login'];
		} else {
		...
		if (isset($this->request->post['unitpay_key'])) {
			$this->data['unitpay_key'] = $this->request->post['unitpay_key'];
		} else {
		...
		if (isset($this->request->post['unitpay_order_status_id'])) {
			$this->data['unitpay_order_status_id'] = $this->request->post['unitpay_order_status_id'];
		} else {
			$this->data['unitpay_order_status_id'] = $this->config->get('unitpay_order_status_id');
		}
		...
		if (isset($this->request->post['unitpay_geo_zone_id'])) {
			$this->data['unitpay_geo_zone_id'] = $this->request->post['unitpay_geo_zone_id'];
		} else {
			$this->data['unitpay_geo_zone_id'] = $this->config->get('unitpay_geo_zone_id');
		}
		...
		if (isset($this->request->post['unitpay_status'])) {
			$this->data['unitpay_status'] = $this->request->post['unitpay_status'];
		} else {
			$this->data['unitpay_status'] = $this->config->get('unitpay_status');
		}

		if (isset($this->request->post['unitpay_sort_order'])) {
			$this->data['unitpay_sort_order'] = $this->request->post['unitpay_sort_order'];
		} else {
			$this->data['unitpay_sort_order'] = $this->config->get('unitpay_sort_order');
		}

		$this->template = 'payment/unitpay.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
